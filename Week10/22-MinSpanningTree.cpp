		...
		while ( !EdgeQ.empty() ) {
			...
			while ( (*MST)[i] ) {
				...
				while ( tempE ) {
					...
					if ( tempE->getData()->getAnotherEnd(nowV)->getData() == (*MST)[i]->getData() && tempE->getData()->getData() == nowedge.first ) {
						if ( !Treedata.count(nowV) ) {

							Treedata.insert(nowV);
							MST->addVertex(nowV->getData());
							int k = 0;
							WeightedGraphVertex<V, E>* t;
							while ( (*MST)[k] ) {
								if ( (*MST)[k]->getData() == nowV->getData() ) t = (*MST)[k];
								k++;
							} 
							MST->addLink( t, (*MST)[i], nowedge.first);
							control = true;
							break;
						}
