...
public:
	...
	*/
	WeightedGraph *shortestPathTree(WeightedGraphVertex<V, E> *v)
	{
		...
		set<WeightedGraphVertex<V, E>*> Treedata;
		priority_queue< pair< E, WeightedGraphVertex<V, E>* > , vector<pair<E, WeightedGraphVertex<V, E>*>>, greater< pair<E, WeightedGraphVertex<V, E>*>> > EdgeQ;

		ListNode<WeightedGraphEdge<V, E> *> * cur = (*v)[0];
		while ( cur ) {
			EdgeQ.push( make_pair( cur->getData()->getData(), cur->getData()->getAnotherEnd(v) ) );
			cur = cur->getNext();
		}

		SPT->addVertex(v->getData());
		Treedata.insert(v);

		while ( !EdgeQ.empty() ) {

			auto nowedge = EdgeQ.top();
			EdgeQ.pop();

			WeightedGraphVertex<V, E> *nowV = nowedge.second;
			
			if ( Treedata.count(nowV) ) continue;
			...
			while ( (*SPT)[i] ) {
				...
				ListNode<WeightedGraphEdge<V, E>*> *tempE = (*nowV)[0];
				while ( tempE ) {
					
					if ( tempE->getData()->getAnotherEnd(nowV)->getData() == (*SPT)[i]->getData() && tempE->getData()->getData() == nowedge.first ) {
						if ( !Treedata.count(nowV) ) {

							Treedata.insert(nowV);
							SPT->addVertex(nowV->getData());
							int k = 0;
							WeightedGraphVertex<V, E>* t;
							while ( (*SPT)[k] ) {
								if ( (*SPT)[k]->getData() == nowV->getData() ) t = (*SPT)[k];
								k++;
							} 
							SPT->addLink( t, (*SPT)[i], nowedge.first);
							control = true;
							break;
						}
			...
			ListNode<WeightedGraphEdge<V, E>*> *tempE = (*nowV)[0];
			while ( tempE ) {
				EdgeQ.push(make_pair(tempE->getData()->getData(),tempE->getData()->getAnotherEnd(nowV)));
				tempE = tempE->getNext();
			}
