	...
	*/
	bool isForest() {
        ...
        try {
            int i = 0;
            while(true) {
                GraphNode<T>* cur = (*vertex)[i].getData();
                if (!visited[cur]) {
                    if (!DFS(cur, nullptr, visited)) {
                        return false;
                    }
                }
                i++;
    ...
    bool DFS(GraphNode<T>* current, GraphNode<T>* parent, map<GraphNode<T>*, bool>& visited) {
        if (!current) return true;
        
        // Mark current node as visited
        visited[current] = true;
        
        // Check all vertices if they're connected to current
        try {
            int i = 0;
            while(true) {
                GraphNode<T>* neighbor = (*vertex)[i].getData();
                
                // If this vertex is connected to current and is not the parent
                if (neighbor != parent && current->exist(neighbor)) {
                    // If neighbor is already visited, we found a cycle
                    if (visited[neighbor]) {
                        return false;
                    }
                    // If neighbor is not visited, recursively check it
                    else if (!DFS(neighbor, current, visited)) {
                        return false;
                    }
                }
                i++;
